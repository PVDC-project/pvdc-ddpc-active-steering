...
if (t == 0)
    ...
    % objective function
    lambda_alpha = 0.25;  % regularization term on alpha
    lambda_slack = 1e6;  % regularization term on the slack variable
    objective = 0;
    ...
    parameters_in = {u0,y0,Hopt,u(2,n+1:L+n)};  % current "state" through past measurements, PE trajectory, curvature preview
...
% select trajectory based on current speed
...
uopt = U(1,n+1);  % select the first predicted input
