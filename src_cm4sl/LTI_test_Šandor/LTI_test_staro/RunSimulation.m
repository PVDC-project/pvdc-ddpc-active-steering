...
T_test = 3;  % [s] duration of the test trajectory
N = T_test/Ts;  % number of samples in the test trajectory
L = 10;  % length of the prediction horizon
% DLCoffset = round(vx*3.6/60*250,-1);  % [m] global X at which the maneuver starts
DLCoffset = 400;
n = 3;  % assumed system dimension, for recording past measurements

nu = 2;  % number of inputs
ny = 2;  % number of outputs
collect_data = 0;  % collect data for the Hankel matrix before the maneuver
...
hold on; plot(X_global(1:length(Y_ref)),Y_ref);
xlim([300 600])