function controller_output = DDPC(controller_input, lambda_alpha, lambda_slack)
...
%     constraints = [constraints, Hopt*alpha == [u(:);y(:)]+[zeros((n+L)*nu,1);slack;zeros(ny*L,1)]];  % "relaxed" model of the data-driven MPC
    ...
    % YALMIP2FORCES
    if ~isfile('./codegen/DDPCf.m') && false
         
        % create folder for solver related files
        solverDir = './codegen';
        ...
        codeoptions = getOptions('DDPCf'); % give solver a name
        codeoptions.printlevel = 0;
        codeoptions.optlevel = 3;    
        
        param_names = {'u0', 'y0', 'Hopt', 'K_seq'};
        sol_names = {'u','y','alpha'};
        
        cd(solverDir);
        optimizerFORCES(constraints, objective, codeoptions, parameters_in, solutions_out, param_names, sol_names);
        cd('../');
    end
end
...
tic;
[solutions, errorcode, errortext, ~, ~, diagnostics] = DDPController(inputs);
...
% alpha = solutions{3};
% disp(['norm(alpha): ',num2str(norm(alpha))])
% disp(['sum(alpha): ',num2str(sum(alpha))])
% disp('---------------')
end